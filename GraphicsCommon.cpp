...
void Graphics::InitShaders(ComPtr<ID3D11Device> &device) {
    ...
    // 정점(slot 0, basicIEs와 동일) + 인스턴스별 world 행렬 4행(slot 1, WORLD0~3)
    ...
    D3D11Utils::CreateVertexShaderAndInputLayout(device, L"NormalVS.hlsl", basicIEs, normalVS,
                                                 basicIL);
...
void Graphics::InitPipelineStates(ComPtr<ID3D11Device> &device) {
    ...
    // instancedSolidPSO: 물리 구 10개를 한 번에 그리는 DrawIndexedInstanced용
    ...
    // reflectInstancedSolidPSO: 반사면 Winding 반대
